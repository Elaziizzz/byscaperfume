...
export default function ReportsPage() {
  ...
  const [activeStore] = useState("bysca");
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(20);
    doc.text(`${"Bysca".toUpperCase()} - P&L Report`, 14, 22);
    ...
    doc.save(`Laporan_${"Bysca"}_${filterLabel.replace(/\s+/g, '_')}.pdf`);
  };
  ...
  const exportExcel = async () => {
    try {
      ...
      const titleRow = sheet.addRow(["LAPORAN KEUANGAN & LABA RUGI - " + "Bysca".toUpperCase()]);
      ...
      saveAs(new Blob([buffer]), `Laporan_Laba_Rugi_${"Bysca"}_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
    } catch (err: any) {
      console.error(err);
